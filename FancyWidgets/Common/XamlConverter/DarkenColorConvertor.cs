...
public class DarkenColorConvertor : IValueConverter
{
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is ISolidColorBrush color && parameter 
                                            ...
                                            && double.TryParse(parameterString, NumberStyles.Any, CultureInfo.InvariantCulture, out var percentage))
        {
            var newR = (byte)(color.Color.R * (1 - percentage));
            var newG = (byte)(color.Color.G * (1 - percentage));
            var newB = (byte)(color.Color.B * (1 - percentage));

            return new SolidColorBrush(new Color(color.Color.A, newR, newG, newB));
        }
